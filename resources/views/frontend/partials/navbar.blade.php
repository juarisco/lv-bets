<nav class="navbar navbar-expand-lg navbar-dark bg-danger py-1 mb-3 mt-3 rounded">
    ...
    <div class="collapse navbar-collapse justify-content-md-center" id="navbarsExample10">
        <ul class="navbar-nav">
            ...
            <li class="nav-item  {{ setActiveRoute('frontend.index') }}">
                <a class="nav-link p-2" href="{{ route('frontend.index') }}">All Today <span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item dropdown {{ setActiveRoute('frontend.showRaffleResults') }}">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdown10">
                    @foreach ($menu_raffles as $raffle)
                        <a class="dropdown-item" href="{{ route('frontend.showRaffleResults', $raffle->slug ) }}">{{ Str::title($raffle->name) }}</a>
                    @endforeach
                </div>
            </li>
            <li class="nav-item">
            ...
            <li class="nav-item dropdown {{ setActiveRoute('frontend.showLotteryResults') }}">
